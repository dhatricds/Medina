...
function ProjectCard({ project, onView, onDelete }: {
  ...
  return (
    ...
    >
      {/* Delete button */}
      <button
        className="absolute top-3 right-3 w-7 h-7 rounded-md flex items-center justify-center opacity-0 group-hover:opacity-100 hover:bg-red-50 transition-all cursor-pointer"
        onClick={(e) => {
          e.stopPropagation();
          if (confirmDelete) {
            onDelete();
...
export default function DashboardView() {
  const { dashboardProjects, viewDashboardProject, removeDashboardProject } = useProjectStore();
